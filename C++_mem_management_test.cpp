...
//其接口雖有標準規格，但實現廠商並未完全遵守；下面三者形式略異。
    ...
    int* p4 = allocator<int>().allocate(3, (int*)0);// (int*)0：无用参数?
    ...
    allocator<int>().deallocate(p4,5);
        ...
        void* p4 = allocator<int>().allocate(7); // allocator<int>()创建临时对象，且未命名
        allocator<int>().deallocate((int*)p4,7);// deallocate时需要指定个数
...
namespace jj07
{
    ...
    public:
        ...
        //(1) 這個就是一般的 operator new() 的重載 -- new class_name()
        void* operator new(size_t size) {
            cout << "operator new(size_t size), size= " << size << endl;
            return malloc(size);
        }

        //(2) 這個就是標準庫已經提供的 placement new() 的重載 (形式)-- new (表示分配地址等参数)class_name
